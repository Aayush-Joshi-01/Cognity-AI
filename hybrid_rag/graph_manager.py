"""
Neo4j Graph Manager with Microsoft GraphRAG patterns:
  - Entity/Relation CRUD with confidence-aware MERGE
  - Community detection via Leiden algorithm (Neo4j GDS)
  - Hierarchical community levels
  - Subgraph retrieval + community-based global search
  - Source lifecycle (confirm/deprecate/prune)
"""

from neo4j import GraphDatabase
from models import Entity, Relation, RetrievalResult, CommunityInfo
from config import Neo4jConfig, GraphRAGConfig


class GraphManager:
    def __init__(self, neo4j_config: Neo4jConfig, graphrag_config: GraphRAGConfig):
    ...
    def upsert_entity(self, entity: Entity):
        """
        MERGE on name. On match: keep higher confidence, merge properties,
        accumulate mentions. Dynamic secondary label from entity_type.
        """
    ...
    # ── Chunk-Entity linking (graph-vector bridge) ──────────────────────

    def link_chunk_to_entities(self, chunk_id: str, doc_id: str, entity_names: list[str]):
        """Create ChunkRef node and MENTIONED_IN edges → bridges vector and graph."""
        with self.driver.session(database=self.db) as s:
            s.run("""
                MERGE (ch:ChunkRef {chunk_id: $chunk_id})
    ...
    def remove_doc_subgraph(self, doc_id: str):
        with self.driver.session(database=self.db) as s:
            # Remove chunk refs
            s.run("""
                MATCH (ch:ChunkRef {doc_id: $doc_id})
                DETACH DELETE ch
            """, doc_id=doc_id)
            # Remove relations exclusive to this doc
            s.run("MATCH ()-[r]->() WHERE r.source_id = $doc_id DELETE r", doc_id=doc_id)
            # Remove orphan entities (no remaining edges)
            s.run("""
                MATCH (e:Entity {source_id: $doc_id})
                WHERE NOT (e)--()
                DELETE e
            """, doc_id=doc_id)
    ...
    # ══════════════════════════════════════════════════════════════════════
    # COMMUNITY DETECTION (Microsoft GraphRAG Leiden)
    # ══════════════════════════════════════════════════════════════════════

    def detect_communities(self) -> list[dict]:
        """
        Run Leiden community detection via Neo4j GDS.
        Creates hierarchical community assignments on Entity nodes.

        Requires: Neo4j GDS plugin enabled.
        Fallback: Louvain if Leiden unavailable.
        """
        with self.driver.session(database=self.db) as s:
            # Project the graph
            try:
            ...
            try:
                ...
                """, max_levels=self.grag.max_community_levels,
                     resolution=self.grag.leiden_resolution)
            except Exception:
                result = s.run("""
    ...
    def get_community_entities(self, community_id: int) -> list[dict]:
        """Get all entities and their inter-relations within a community."""
        with self.driver.session(database=self.db) as s:
            result = s.run("""
    ...
    def store_community_summary(self, community: CommunityInfo):
        """Persist community summary as a node for global search."""
        with self.driver.session(database=self.db) as s:
            s.run("""
    ...
    def retrieve_subgraph(self, entity_names: list[str], hops: int = 2,
        ...
        """Local search: BFS from seed entities."""
        query = """
    ...
    def retrieve_entity_context(self, entity_name: str) -> list[RetrievalResult]:
        """Get full context for a single entity: description + all relations."""
        query = """
        MATCH (e:Entity)
    ...
    def global_community_search(self, top_n: int = None) -> list[RetrievalResult]:
        """GraphRAG global search: return community summaries ranked by importance."""
        top_n = top_n or self.grag.global_search_top_communities
        query = """
        MATCH (c:Community)
    ...
    # ── Entity-to-Chunk bridge retrieval ────────────────────────────────

    def get_chunks_for_entities(self, entity_names: list[str]) -> list[str]:
        """Find chunk IDs linked to given entities (graph→vector bridge)."""
        query = """
    ...
    def deprecate_source(self, doc_id: str):
        with self.driver.session(database=self.db) as s:
            ...
            s.run("MATCH ()-[r]->() WHERE r.source_id = $d SET r.confidence = r.confidence * 0.5", d=doc_id)
