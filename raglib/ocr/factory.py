...
class OCRFactory:
    ...
    def create(provider: str, config=None) -> BaseOCR:
        ...
        if provider == "gemini_vision":
            from raglib.ocr.gemini_vision import GeminiVisionOCR
        ...
        if provider == "openai_vision":
            from raglib.ocr.openai_vision import OpenAIVisionOCR
        ...
        if provider == "anthropic_vision":
            from raglib.ocr.anthropic_vision import AnthropicVisionOCR
        ...
        if provider == "azure_vision":
            from raglib.ocr.azure_vision import AzureVisionOCR
            if config:
        ...
        if provider == "bedrock_vision":
            from raglib.ocr.bedrock_vision import BedrockVisionOCR
            if config:
        ...
        if provider == "tesseract":
            from raglib.ocr.tesseract import TesseractOCR
            return TesseractOCR()
