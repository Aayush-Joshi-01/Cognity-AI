...
def _build_embedder(key: str, cfg: LibraryConfig):
    if key == "gemini":
        ...
        from cognity_ai.embedders.vertex_ai import VertexAIEmbedder
        ...
        from cognity_ai.embedders.ollama import OllamaEmbedder
...
def _build_generator(key: str, cfg: LibraryConfig):
    if key == "gemini":
        ...
        from cognity_ai.generators.vertex_ai import VertexAIGenerator
        ...
        from cognity_ai.generators.ollama import OllamaGenerator
...
def _build_vector_store(cfg: LibraryConfig):
    ...
    if key == "chroma":
        ...
        from cognity_ai.stores.vector.weaviate import WeaviateStore
        return WeaviateStore(cfg.weaviate)
    elif key == "milvus":
        from cognity_ai.stores.vector.milvus import MilvusStore
        return MilvusStore(cfg.milvus)
    elif key == "pgvector":
        from cognity_ai.stores.vector.pgvector import PgVectorStore
        return PgVectorStore(cfg.pgvector)
    elif key == "azure_search":
        from cognity_ai.stores.vector.azure_search import AzureSearchStore
...
def _build_extractor(cfg: LibraryConfig, nlp_model, generator):
    ...
    if mode == "nlp_only":
        if nlp_model is None:
            ...
            from cognity_ai.extractors.llm import LLMExtractor
            return LLMExtractor(generator)
        from cognity_ai.extractors.nlp import NLPExtractor
        return NLPExtractor(nlp_model, cfg.nlp)
    elif mode == "llm_only":
        from cognity_ai.extractors.llm import LLMExtractor
        return LLMExtractor(generator)
    else:  # "hybrid" (default)
        from cognity_ai.extractors.hybrid import HybridExtractor
        from cognity_ai.extractors.nlp import NLPExtractor
        from cognity_ai.extractors.llm import LLMExtractor
