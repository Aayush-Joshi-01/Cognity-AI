...
class LoaderFactory:
    ...
    def get_loader(cls, path: str, ocr_provider=None) -> BaseLoader:
        ...
        if loader_name == "text":
            from cognity_ai.loaders.text import TxtLoader
            return TxtLoader()

        if loader_name == "markdown":
            from cognity_ai.loaders.text import MdLoader
            return MdLoader()
        ...
        if loader_name == "excel":
            from cognity_ai.loaders.excel import ExcelLoader
            return ExcelLoader()
        ...
        if loader_name == "pptx":
            from cognity_ai.loaders.pptx import PptxLoader
            return PptxLoader()
        ...
        if loader_name == "json":
            from cognity_ai.loaders.json_loader import JsonLoader
            return JsonLoader()

        if loader_name == "yaml":
            from cognity_ai.loaders.json_loader import YamlLoader
            return YamlLoader()

        if loader_name == "image":
            from cognity_ai.loaders.image import ImageLoader
